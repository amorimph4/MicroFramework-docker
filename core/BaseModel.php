...
abstract class BaseModel
{
    ...
    public function findBy(array $data, $columns = "*", $all = false)
    {
        $data = $this->prepareDataWhere($data);
        $query = "SELECT $columns FROM {$this->table} WHERE {$data[0]}";
        $stmt = $this->pdo->prepare($query);
    ...
    private function prepareDataWhere(array $data)
    {
        ...
        foreach ($data as $key => $value){
            $strKeysBinds = "{$strKeysBinds} AND {$key}=:{$key}";
            $binds[] = ":{$key}";
            $values[] = $value;
        }
    ...
    public function deleteBy(array $data)
    {
        if (0 < count($data)) {
            $data = $this->prepareDataWhere($data);
            $query = "DELETE FROM {$this->table} WHERE {$data[0]}";
            $stmt = $this->pdo->prepare($query);
            for($i = 0; $i < count($data[1]); $i++){
                $stmt->bindValue("{$data[1][$i]}", $data[2][$i]);
            }
